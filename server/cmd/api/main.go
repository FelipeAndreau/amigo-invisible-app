...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		protected.Use(auth.AuthMiddleware())
		{
			...
			protected.POST("/events/:id/preferences", event.SavePreferencesHandler)
			protected.GET("/events/:id/my-assignment/preferences", event.GetAssignmentPreferencesHandler)
			protected.POST("/events/:id/gift-progress", event.SaveGiftProgressHandler)
			protected.GET("/events/:id/gift-progress", event.GetGiftProgressHandler)
			protected.PATCH("/events/:id/date", event.SetEventDateHandler)
			protected.GET("/events/:id/date", event.GetEventDateHandler)
			protected.POST("/events/:id/gallery", event.UploadGalleryPhotoHandler)
			protected.GET("/events/:id/gallery", event.ListGalleryPhotosHandler)
			protected.GET("/events/history", event.GetHistoryHandler)
